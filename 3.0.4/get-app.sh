...
function install_9hits() {
	...
	wget -4 -O "$INSTALL_DIR/_9hits.tar.bz2" $DOWNLOAD_URL
	if [ $? -ne 0 ]; then
		echo "Failed to download 9Hits App. Exiting."
		exit 1
	fi
	
	echo "Extracting the 9Hits App..."
	tar -xjvf "$INSTALL_DIR/_9hits.tar.bz2" -C "$INSTALL_DIR"
	if [ $? -ne 0 ]; then
		echo "Failed to extract 9Hits App. Exiting."
		exit 1
	fi
